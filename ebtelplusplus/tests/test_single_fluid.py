...
@pytest.mark.parametrize('event', [
    HeatingEvent(0*u.s, 2000*u.s, 250*u.s, 1000*u.s, 0.005*u.Unit('erg cm-3 s-1')),  # gentle case
    HeatingEvent(0*u.s, 200*u.s, 100*u.s, 100*u.s,  0.1*u.Unit('erg cm-3 s-1')),  # more impulsive case
])
def test_single_fluid_gentle(physics_model, event):
    heating = HeatingModel(background=1e-6*u.Unit('erg cm-3 s-1'),
                           partition=0.5,
                           events=[event])
    results = ebtelplusplus.run(5e3*u.s, 40*u.Mm, heating=heating, physics=physics_model)
