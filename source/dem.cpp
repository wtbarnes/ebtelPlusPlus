...
Dem::Dem(LOOP loop_object)
{
  ...
  for(int i=0;i<nbins;i++)
  {
    ...
    __radiative_loss[i] = loop->CalculateRadiativeLosses(__temperature[i]);
  }
  // Resize DEM arrays
...
void Dem::CalculateDEM(int i)
{
  ...
  double R_tr = loop->CalculateC1(loop_state[3],loop_state[4],loop_state[2])*pow(loop_state[2],2)*loop->CalculateRadiativeLosses(loop_state[3])*loop->parameters.loop_length;
